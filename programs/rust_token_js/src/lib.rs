...
pub mod rust_token_js {
    ...
    pub fn proxy_mint(ctx: Context<ProxyMintTo>, amount: u64) -> Result<()> {
    ...
    pub fn proxy_authority_set(
        ctx: Context<ProxySetAuthority>,
        authority_type: AuthorityType,
        new_authorty: Option<Pubkey>,
    ) -> Result<()> {
        token::set_authority(ctx.accounts.into(), authority_type.into(), new_authorty)
    }
}
